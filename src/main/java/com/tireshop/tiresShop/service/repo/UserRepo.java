...
public class UserRepo {
    ...
    public ResponseEntity<String> deleteItemInShoppingCart(Long userId, int itemId) {
        ...
        int rowsAffected = jdbcTemplate.update(deleteSql, itemId);
    ...
    public ResponseEntity<UpdateResponse> updateItemInShoppingCart(Long userId, int itemId, int quantity) {
        ...
        if (userCount == false) {
            return new ResponseEntity<>(new UpdateResponse("User not found", null), HttpStatus.NOT_FOUND);

        }

        // Update item's quantity from shopping cart
        String deleteSql = "UPDATE user_basket set quantity = ? where id = ?";
        int rowsAffected = jdbcTemplate.update(deleteSql, quantity, itemId);

        // Get updated item from shopping cart
        String getItemSql = """
                    SELECT
                    user_basket.id,
                    user_basket.user_id,
        ...
        List<UsersCartItems> updatedItem = jdbcTemplate.query(getItemSql, mapper,
                itemId);

        if (rowsAffected > 0 && !updatedItem.isEmpty()) {
            return new ResponseEntity<>(
                    new UpdateResponse("Item's quantity has been successfully updated", updatedItem),
                    HttpStatus.OK);
        } else {
            return new ResponseEntity<>(new UpdateResponse("Item not found in shopping cart", null),
                    HttpStatus.NOT_FOUND);
        }
    }
}
